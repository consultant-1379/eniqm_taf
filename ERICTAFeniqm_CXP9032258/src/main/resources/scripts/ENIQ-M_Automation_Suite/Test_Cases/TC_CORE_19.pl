...
my $TCID="TC_CORE_19";
my $TC_Name="Check Topology file for IMS-M node";
my $TC_Result;
my $Total_time=0;
...
my @NODES;
my $NODE_TXT="/home/nmsadm/ENIQ-M_Automation_Suite/Nodes/IMS-M.txt";
my $Topology_File;
my $T_PATH;
my $S_PATH;
my $EV_S_PATH;
...
sub testcase()
{
	...
	print FT Date(),">>> Checking Topology file for IMS-M nodes";
	print FT "=============================================";
	
	print FT Date(),"Reading IMS-M node info from '$NODE_TXT'";
	...
	foreach(@NODES)
	{
		...
		$T_PATH="$ENIQ_VOLUME_MT_POINT/outputfiles/core/topologyData/CoreNetwork/$Topology_File";
		...
		print FT "NO nodes of this nodetype are not present in OSS server";
	}	
	};
