...
my $TCID="TC_GRAN_9";
my $TC_Name="Check for TRC topology file";
my $TC_Result;
my $Total_time=0;
...
my $TRC_txt="/home/nmsadm/ENIQ-M_Automation_Suite/Nodes/TRC.txt";
my $TRC_T_FILE="$ENIQ_VOLUME_MT_POINT/outputfiles/gran/topologyData/GranNetwork/TRC";
my $TRC_S_SymLink="$ENIQ_VOLUME_MT_POINT/gran/topologyData/GranNetwork/TRC";
my $TRC_EV_SymLink="$ENIQ_ENIQM_LOC_PATH/gran/topologyData/GranNetwork/TRC";
...
sub testcase()
{
	...
	if (-e $TRC_txt)
	{
		if(-z $TRC_txt)
		{
			print FT "No TRC nodes are present in OSS server";
		}else{
		print FT Date(),">>> Checking TRC topology file";
		print FT ENIQM_Lib::Check_Topology_file($TRC_T_FILE);
		print FT Date(),">>> Checking the Headder part of TRC Topology file";
		Check_Headder();
		print FT Date(),">>> Checking Stats symbolic link";
		print FT ENIQM_Lib::Check_Symbolic_Link($TRC_S_SymLink);
		print FT Date(),">>> Checking Events symbolic link";
		print FT ENIQM_Lib::Check_Symbolic_Link($TRC_EV_SymLink);
		}
	}else{
	$EXIT_CODE=12;
	die "No such file found '$TRC_txt'";
	}
	};
...
sub Check_Headder{
my @FILE_Content;
open(FH,"$TRC_T_FILE"),or $EXIT_CODE=12,die "'$TRC_T_FILE' No such file found\n";
@FILE_Content=<FH>;
close FH;
