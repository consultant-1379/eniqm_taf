...
my $TCID="TC_GRAN_2";
my $TC_Name="Check for MCTR topology file";
my $TC_Result;
my $Total_time=0;
...
my $MCTR_T_FILE="$ENIQ_VOLUME_MT_POINT/outputfiles/gran/topologyData/MCTR/MCTR";
my $MCTR_S_SymLink="$ENIQ_VOLUME_MT_POINT/gran/topologyData/MCTR/MCTR";
my $MCTR_EV_SymLink="$ENIQ_ENIQM_LOC_PATH/gran/topologyData/MCTR/MCTR";
...
sub testcase()
{
	...
	################################ Testcase Execution #########################################
	print FT Date(),">>> Checking MCTR topology file";
	print FT ENIQM_Lib::Check_Topology_file($MCTR_T_FILE);
	print FT Date(),">>> Checking the Headder part of MCTR Topology file";
	Check_Headder();
	print FT Date(),">>> Checking Stats symbolic link";
	print FT ENIQM_Lib::Check_Symbolic_Link($MCTR_S_SymLink);
	print FT Date(),">>> Checking Events symbolic link";
	print FT ENIQM_Lib::Check_Symbolic_Link($MCTR_EV_SymLink);
...
sub Check_Headder{
my @FILE_Content;
open(FH,"$MCTR_T_FILE"),or $EXIT_CODE=12,die "'$MCTR_T_FILE' No such file found\n";
@FILE_Content=<FH>;
close FH;
