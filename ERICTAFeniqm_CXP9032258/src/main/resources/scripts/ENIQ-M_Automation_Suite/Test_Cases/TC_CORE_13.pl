...
my $TCID="TC_CORE_13";
my $TC_Name="Check Topology file for HLR node";
my $TC_Result;
my $Total_time=0;
...
my @NODES;
my $NODE_TXT="/home/nmsadm/ENIQ-M_Automation_Suite/Nodes/HLR.txt";
my $Topology_File;
my $T_PATH;
my $S_PATH;
my $HLR_s_PATH;
...
sub testcase()
{
	...
	print FT Date(),">>> Checking Topology file for HLR nodes";
	print FT "=============================================";
	
	print FT Date(),"Reading HLR node info from '$NODE_TXT'";
	...
	foreach(@NODES)
	{
		...
		$T_PATH="$ENIQ_VOLUME_MT_POINT/outputfiles/core/topologyData/AXE/$Topology_File";
		
		if($T_PATH=~m/$ENIQ_VOLUME_MT_POINT\/outputfiles\/(.*)/)
		{
			$S_PATH="$ENIQ_VOLUME_MT_POINT/$1";
			$HLR_s_PATH="$ENIQ_VOLUME_MT_POINT/outputfiles/core/topologyData/AXE/SymLink/HLR/$Topology_File";
		...
		print FT ENIQM_Lib::Check_Symbolic_Link($HLR_s_PATH);
