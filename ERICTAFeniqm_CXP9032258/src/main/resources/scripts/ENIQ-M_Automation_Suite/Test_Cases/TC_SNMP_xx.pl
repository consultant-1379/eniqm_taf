...
my $TCID="TC_SNMP_10";
my $TC_Name="Check Topology file for CS-MS node";
my $TC_Result;
my $Total_time=0;
my $Start_time=`date +%H:%M:%S`;
my $End_time=0;
my $EXIT_CODE=0;


my @NODES;
my $NODE_TXT="/home/nmsadm/ENIQ-M_Automation_Suite/Nodes/CS-MS.txt";
my $Topology_File;
my $T_PATH;
my $S_PATH;
my $EV_S_PATH;
...
sub testcase()
{
	...
	print FT Date(),">>> Checking Topology file for CS-MS nodes";
	print FT "=============================================";
	
	print FT Date(),"Reading CS-MS node info from '$NODE_TXT'";
	...
	foreach(@NODES)
	{
		...
		$T_PATH="$ENIQ_VOLUME_MT_POINT/outputfiles/snmp/topologyData/Nodes/$Topology_File";
