...
my $TCID="TC_GRAN_6";
my $TC_Name="Check for BSC topology file";
my $TC_Result;
my $Total_time=0;
...
my $BSC_txt="/home/nmsadm/ENIQ-M_Automation_Suite/Nodes/BSC.txt";
my $BSC_T_FILE="$ENIQ_VOLUME_MT_POINT/outputfiles/gran/topologyData/GranNetwork/BSC";
my $BSC_S_SymLink="$ENIQ_VOLUME_MT_POINT/gran/topologyData/GranNetwork/BSC";
my $BSC_EV_SymLink="$ENIQ_ENIQM_LOC_PATH/gran/topologyData/GranNetwork/BSC";
...
sub testcase()
{
	...
	if (-e $BSC_txt)
	{
		if(-z $BSC_txt)
		{
			print FT "No BSC nodes are present in OSS server";
		}else{
		print FT Date(),">>> Checking BSC topology file";
		print FT ENIQM_Lib::Check_Topology_file($BSC_T_FILE);
		print FT Date(),">>> Checking the Headder part of BSC Topology file";
		Check_Headder();
		print FT Date(),">>> Checking Stats symbolic link";
		print FT ENIQM_Lib::Check_Symbolic_Link($BSC_S_SymLink);
		print FT Date(),">>> Checking Events symbolic link";
		print FT ENIQM_Lib::Check_Symbolic_Link($BSC_EV_SymLink);
		}
	}else{
	$EXIT_CODE=12;
	die "No such file found '$BSC_txt'";
	}
	};
...
sub Check_Headder{
my @FILE_Content;
open(FH,"$BSC_T_FILE"),or $EXIT_CODE=12,die "'$BSC_T_FILE' No such file found\n";
@FILE_Content=<FH>;
close FH;
