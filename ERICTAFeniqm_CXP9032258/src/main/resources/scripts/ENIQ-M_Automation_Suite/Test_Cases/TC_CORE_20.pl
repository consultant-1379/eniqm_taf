...
my $TCID="TC_CORE_20";
my $TC_Name="Check Topology file for VLR node";
my $TC_Result;
my $Total_time=0;
...
my $VLR_s_PATH;
my @NODES;
my $NODE_TXT="/home/nmsadm/ENIQ-M_Automation_Suite/Nodes/VLR.txt";
my $Topology_File;
my $T_PATH;
my $S_PATH;
my $EV_S_PATH;
...
sub testcase()
{
	...
	print FT Date(),">>> Checking Topology file for VLR nodes";
	print FT "=============================================";
	
	print FT Date(),"Reading VLR node info from '$NODE_TXT'";
	...
	foreach(@NODES)
	{
		...
		if($T_PATH=~m/$ENIQ_VOLUME_MT_POINT\/outputfiles\/(.*)/)
		{
			...
			$VLR_s_PATH="$ENIQ_VOLUME_MT_POINT/outputfiles/core/topologyData/AXE/SymLink/VLR/$Topology_File";
		}else{
			$EXIT_CODE=12;
		...
		print FT ENIQM_Lib::Check_Symbolic_Link($VLR_s_PATH);
	}		
	}else{
