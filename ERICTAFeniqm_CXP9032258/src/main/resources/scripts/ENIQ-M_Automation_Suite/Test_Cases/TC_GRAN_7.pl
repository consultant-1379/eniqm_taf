...
my $TCID="TC_GRAN_7";
my $TC_Name="Check for SMPC topology file";
my $TC_Result;
my $Total_time=0;
...
my $SMPC_txt="/home/nmsadm/ENIQ-M_Automation_Suite/Nodes/SMPC.txt";
my $SMPC_T_FILE="$ENIQ_VOLUME_MT_POINT/outputfiles/gran/topologyData/GranNetwork/SMPC";
my $SMPC_S_SymLink="$ENIQ_VOLUME_MT_POINT/gran/topologyData/GranNetwork/SMPC";
my $SMPC_EV_SymLink="$ENIQ_ENIQM_LOC_PATH/gran/topologyData/GranNetwork/SMPC";
...
sub testcase()
{
	...
	if (-e $SMPC_txt)
	{
		if(-z $SMPC_txt)
		{
			print FT "No SMPC nodes are present in OSS server";
		}else{
		print FT Date(),">>> Checking SMPC topology file";
		print FT ENIQM_Lib::Check_Topology_file($SMPC_T_FILE);
		print FT Date(),">>> Checking the Headder part of SMPC Topology file";
		Check_Headder();
		print FT Date(),">>> Checking Stats symbolic link";
		print FT ENIQM_Lib::Check_Symbolic_Link($SMPC_S_SymLink);
		print FT Date(),">>> Checking Events symbolic link";
		print FT ENIQM_Lib::Check_Symbolic_Link($SMPC_EV_SymLink);
		}
	}else{
	$EXIT_CODE=12;
	die "No such file found '$SMPC_txt'";
	}
	};
...
sub Check_Headder{
my @FILE_Content;
open(FH,"$SMPC_T_FILE"),or $EXIT_CODE=12,die "'$SMPC_T_FILE' No such file found\n";
@FILE_Content=<FH>;
close FH;
